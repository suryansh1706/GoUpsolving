...
This project uses **Vite** for the frontend and **Vercel Functions** for the backend API.

### Prerequisites

- Node.js 16+ and npm
- Vercel CLI (for local API development): `npm install -g vercel`
...
2. **Start the Vercel dev server in one terminal:**
   ```bash
   vercel dev
   ```
...
4. **Open your browser:**
   ...
   - The Vite dev server will proxy API calls to the Vercel dev server

### Troubleshooting: "invalid json" Error

If you see the error: `unexpected token 'c', "const code"... is not valid json`

**This means the API server is not running!**

**Solution:**
1. Ensure you're running `vercel dev` in a separate terminal
2. Check that it's running on port 3000
3. Try refreshing the page after starting `vercel dev`
